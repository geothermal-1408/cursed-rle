...
static int rle_max_encoded_size(int pixel_count, int channels)
{
  if (pixel_count <= 0 || channels <= 0)
    return -1;

  if (pixel_count > INT_MAX / (channels + 1))
    return -1;

  return pixel_count * (channels + 1);
}
...
void handle_input_screen(const char *title, int is_encode)
{
  ...
  char *res = NULL;
  if (is_encode)
    res = rle_encode(input_buffer);
  else
    res = rle_decode(input_buffer);
...
pick_bmp:
  ...
  */
  ...
  int pixel_count = w * h;
  int max_encoded_len = rle_max_encoded_size(pixel_count, n);

  if (max_encoded_len <= 0)
  {
    mvprintw(9, 4, "Error: invalid image size/channel combination");
    mvprintw(11, 4, "press any key to return");
  ...
  unsigned char *encoded = malloc((size_t)max_encoded_len);
  if (!encoded)
  {
    mvprintw(9, 4, "Error: out of memory");
    mvprintw(11, 4, "press any key to return");
    stbi_image_free(data);
    getch();
    return;
  ...
  int enc_length = rle_encode_binary(data, pixel_data_len, encoded, max_encoded_len, n);
  if (enc_length < 0)
  {
    mvprintw(9, 4, "Error: encoding failed");
    mvprintw(11, 4, "press any key to return");
    free(encoded);
    stbi_image_free(data);
    getch();
...
pick_bmp:
  ...
  */
  ...
  if (res != 0)
  {
    ...
    if (res == -2)
      mvprintw(10, 4, "not a vaild .rle file");
    if (res == -3)
      mvprintw(10, 4, "unsupported version");
    if (res == -4)
      mvprintw(10, 4, "out of memory");
    if (res == -5)
      mvprintw(10, 4, "invalid/corrupt file size or read failure");
    mvprintw(12, 4, "press any key to return");
    getch();
    return;
  }
...
int main(void)
{
  ...
  while (1)
  {
    ...
    case KEY_DOWN:
      highlight++;
      if (highlight >= n_choice)
        highlight = n_choice - 1;
      break;

    case 10:
      choice = highlight;
      if (choice == 0)
        handle_input_screen("Encode String", 1);
      else if (choice == 1)
        handle_input_screen("Decode String", 0);
      else if (choice == 2)
        handle_compress_screen();
      else if (choice == 3)
